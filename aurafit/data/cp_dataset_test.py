...
_Variant = Literal["condition", "generator"]

class CPDatasetTest(data.Dataset):
    def __init__(self, opt: Any, variant: _Variant = "generator") -> None:
        super(CPDatasetTest, self).__init__()
        if variant not in ("condition", "generator"):
            raise ValueError(
                f"variant must be 'condition' or 'generator', got {variant!r}"
            )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        if self.variant == "condition":
            return self._getitem_condition(index)
        return self._getitem_generator(index)

    def _getitem_condition(self, index: int) -> Dict[str, Any]:
    ...
    def _getitem_generator(self, index: int) -> Dict[str, Any]:
        im_name = self.im_names[index]
        c_name: Dict[str, str] = {}
        c: Dict[str, torch.Tensor] = {}
        cm: Dict[str, torch.Tensor] = {}
